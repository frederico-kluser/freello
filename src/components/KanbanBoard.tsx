...
export function KanbanBoard({ board, settings, onChange, onOpenSettings, onReset }: Props) {
    ...
    <div className="h-full flex flex-col">
      <header className="px-6 py-4 border-b border-slate-800 flex items-center justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-400 truncate max-w-3xl mt-0.5" title={board.meta.objective}>
            {board.meta.objective}
          </p>
        </div>
      ...
      <footer className="px-6 py-2 border-t border-slate-800/60 text-[10px] text-slate-500 flex justify-between">
        <span>{Object.keys(board.cards).length} cards · drafted with {settings.openRouterModel}</span>
        <span>
          GIFs powered by{' '}
          <a
